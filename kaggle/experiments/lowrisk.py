"""
=====================================================================
CRSD (package `crsd`) — Kaggle OFFLINE notebook (Internet OFF, GPU ON) — exp_lowrisk (đáy thấp 0/1/5%) | 7 MODEL OPEN-SOURCE
=====================================================================
Chạy experiment `exp_lowrisk` — GIỐNG HỆT `exp_riskframing`, CHỈ KHÁC MỨC RỦI RO —
trên 7 model mã nguồn mở đã dùng xuyên suốt project (Qwen2.5 7/32/72B · Gemma-2 9/27B
· Llama-3.1 8/70B). Cùng MỘT cấu hình prompt cố định như turn-3:
...
Khác biệt DUY NHẤT so với exp_riskframing: quét ĐÁY THẤP 0% / 1% / 5% thay vì
90/50/10%. Game config `crsd_milinski_{0,1,5}pct_risk_plain_computed`. Agent trung
tính, full_history, KHÔNG persona/scratchpad.
3 game (3 risk × 1 điều kiện) × 2 lang (EN+VN) × 10 rep = 60 game/model.

Ý NGHĨA 3 MỨC (endowment 40, target 120, 6 người -> phần công bằng 20/người;
EV khi đào ngũ = 40×(1−p)):
  · 0%  — Ô CONTROL. EV đào ngũ = 40 > 20 và KHÔNG có mặt trái nào cả: thế lưỡng nan
    bị xoá bỏ hoàn toàn. Nếu model VẪN đẩy tới 120 thì hành vi là "bám theo con số
    target được nêu", KHÔNG phải quản trị rủi ro.
  · 1% (EV 39.6) và 5% (EV 38.0) — cùng dự đoán lý thuyết với 10% đã chạy: đào ngũ áp
    đảo. Dùng để dò xem có ngưỡng nhị phân "có/không rủi ro" nằm giữa 0% và 10% không.

⚠️  ĐỌC KẾT QUẢ: ở 0% thì `catastrophe` LUÔN False và `target_reached` mất ý nghĩa
  diễn giải (đạt target chẳng đem lại gì thêm). DV chính phải là `group_total`.
  So sánh trực tiếp với exp_riskframing: cùng seed 12345 nên rep k ở đây khớp CRN
  với rep k ở turn-3 — ghép hai bảng lại được đường 0/1/5/10/50/90%.

Notebook này = bản `riskframing.py` đổi ĐÚNG 1 chỗ: EXPERIMENTS (exp_lowrisk).
MODELS[] + Cell 2/2.5/3/4/5/6/7/8 + runner giữ nguyên -> seed/CRN y hệt.
...
Output: /kaggle/working/crsd_results/<model_short>/<experiment>/{turns.jsonl, games.csv}
  ...
  (risk_framing=plain và show_computed_totals=1 là HẰNG SỐ, không tách nữa).
  DV chính = `group_total` (KHÔNG phải `target_reached` — xem cảnh báo ở trên).
=====================================================================
"""
...
EXPERIMENTS = ["exp_lowrisk"]   # 1 điều kiện: plain + computed-totals, risk 0/1/5%, EN+VN
...
_need = [REPO_ROOT / "crsd" / "runner" / "run_experiment.py",
         ...
         REPO_ROOT / "crsd" / "configs" / "experiment" / "exp_lowrisk.json",
         REPO_ROOT / "crsd" / "configs" / "game" / "crsd_milinski_0pct_risk_plain_computed.json",
         REPO_ROOT / "crsd" / "configs" / "game" / "crsd_milinski_1pct_risk_plain_computed.json",
         REPO_ROOT / "crsd" / "configs" / "game" / "crsd_milinski_5pct_risk_plain_computed.json",
...
if _missing:
    ...
    print("OK — crsd/ + FAIRGAME/ + configs/prompts + exp_lowrisk đầy đủ.")
...
if _frames:
    ...
    # exp_lowrisk CHỈ 1 điều kiện prompt nên `risk_framing`/`show_computed_totals` là
    # hằng số (vẫn để trong idx cũng vô hại); trục phân tích thực sự còn lại là
    # `risk_probability` × `language`.
    idx = [c for c in ["model", "experiment", "persona_set", "language", "memory_mode",
                       "framing", "risk_framing", "show_computed_totals"]
           if c in combined.columns]
    # DV CHÍNH = group_total. Ở risk 0% thì catastrophe luôn False và target_reached
    # mất ý nghĩa diễn giải (đạt target chẳng đem lại gì thêm) -> in group_total TRƯỚC,
    # target_reached chỉ để tham chiếu.
    for _val, _fmt, _label in [("group_total", "{:.1f}", "Group total TRUNG BÌNH (target=120)"),
                               ("target_reached", "{:.2f}", "Reach-rate (THAM CHIẾU — vô nghĩa ở risk=0)")]:
        if _val not in combined.columns:
            continue
        try:
            piv = combined.pivot_table(index=idx, columns="risk_probability",
                                       values=_val, aggfunc="mean")
            print(f"\n=== {_label} theo " + " × ".join(idx) + " × risk ===")
            print(piv.to_string(float_format=lambda x: _fmt.format(x)))
        except Exception as e:  # noqa: BLE001
            print(f"(bỏ qua pivot {_val}:", e, ")")
    print("\nĐỐI CHIẾU turn-3 (exp_riskframing, cùng seed 12345 -> CRN khớp từng rep):")
    print("  group_total ~119–178 và PHẲNG suốt 10/50/90%. Nếu 0% cũng phẳng nốt thì")
    print("  hành vi là 'bám con số target', không phải quản trị rủi ro.")
else:
