"""
=====================================================================
CRSD (package `crsd`) — Kaggle OFFLINE notebook (Internet OFF, GPU ON) — exp_baseline | MODEL LỚN (27B/32B)
=====================================================================
GIỐNG HỆT `kaggle_exp_baseline.py` nhưng cấu hình sẵn cho HAI model LỚN:
  - Qwen2.5-32B-Instruct   (/kaggle/input/models/qwen-lm/qwen2.5/transformers/32b-instruct/1)
  - Gemma-2-27B-it         (/kaggle/input/models/google/gemma-2/transformers/gemma-2-27b-it/2)

Chỉ Cell 1 (MODELS + tham số TP) và header này khác bản gốc; Cell 2→8 giữ nguyên
nên tái dùng đúng runner đã test (`crsd.runner.run_experiment`, `run_games_batched`,
`crsd.dataio.recorder`) — seed/CRN/scratchpad/framing y hệt lúc chạy local & bản 7B/9B.

⚠️  YÊU CẦU GPU (đọc kỹ TRƯỚC khi chạy) ─────────────────────────────
Cấu hình của user: chạy trên Kaggle với GPU = MỘT card RTX PRO 6000 (Blackwell) 96 GB
(1 GPU đơn). Connector nạp trọng số ở bf16 (KHÔNG quantization). Cỡ trọng số:
      Qwen2.5-32B  ≈ 64 GB         Gemma-2-27B  ≈ 54 GB   (chưa kể KV-cache)
  => 96 GB ĐỦ cho cả hai ở bf16 trên 1 GPU -> TP_SIZE = 1 (KHÔNG tensor-parallel,
     chỉ có 1 card). gpu_memory_utilization ~0.92 chừa ~24 GB cho KV-cache.
  => RTX PRO 6000 là Blackwell sm_120: GIỮ VLLM_USE_FLASHINFER_SAMPLER=0 (Cell 2.5
     đã set) và cần vLLM/torch bản mới có kernel sm_120 (CUDA 12.x/cu128).
  => Nếu sau này đổi sang accelerator NHIỀU GPU (vd L4 x4 = 4×24 GB) thì mới đặt
     TP_SIZE = số GPU (vd 4). Trên 1 card 96 GB hiện tại: TP_SIZE = 1.

CÁCH CHẠY:
  1. Tạo notebook Kaggle mới — Accelerator: GPU RTX PRO 6000 (96 GB, 1 card), Internet: OFF.
  2. + Add Input:
       a) Code/Dataset: repo này (chứa CẢ `crsd/` lẫn `FAIRGAME/` ở cùng thư mục gốc).
       b) Model(s): add Qwen2.5-32B-Instruct và Gemma-2-27B-it (mỗi model một input).
  3. Copy file này vào notebook, chia cell theo "# CELL N".
  4. Kiểm tra MODELS[] / TP_SIZE ở Cell 1. Chạy "!ls /kaggle/input/" để xem path thực.
  5. Run lần lượt Cell 1 → 8. (Mỗi model nạp riêng, free GPU giữa hai model.)
...
GPU_UTIL = 0.92           # chỉ dùng cho vllm. Model lớn cần để trọng số vừa; nâng 0.95 nếu vẫn thiếu chỗ.
TP_SIZE = 1               # tensor parallel (vllm). RTX PRO 6000 = 1 GPU 96GB -> 1. Chỉ đặt >1 nếu accelerator NHIỀU GPU (vd L4 x4 -> 4).
BATCH_SIZE = 256          # prompts/forward; vLLM tự lên lịch nội bộ -> 256 an toàn kể cả model lớn (phần dư xếp hàng, không OOM).
...
# game × 2 model lớn), hạ tải bằng 1 trong các cách dưới (KHÔNG cần sửa file config):
...
ENFORCE_EAGER = True        # True = an toàn + tiết kiệm VRAM (không CUDA graph) cho model lớn; False = nhanh hơn nhưng tốn thêm VRAM.
