...
def figure(units, games):
    ...
    cs.use()
    fig = plt.figure(figsize=cs.figsize("full", height_pt=150))
    ...
    for i, m in enumerate(cs.MODEL_ORDER, start=1):
        ...
        ax_c.text(100 * kept + (-1.5 if inside else 1.5), i, f"{100 * kept:.0f}",
                  ...
                  color=cs.WHITE if inside else cs.INK, zorder=5,
...
def main():
    ...
    family = [cd.show(m).split(" ", 1)[0] if " " in cd.show(m) else "" for m in MODELS]
    member = [cd.show(m).split(" ", 1)[1] if " " in cd.show(m) else cd.show(m) for m in MODELS]
    tab = [
        "% Generated by paper/AAMAS/analysis/selfplay.py. Do not edit by hand.",
