...
\subtitle{Model lớn (scaling) \enspace\&\enspace Đọc-hiểu (comprehension) \enspace\&\enspace Model frontier đóng}
\date{\today}
...
\begin{frame}{Nhắc lại: Turn trước ba model nhỏ chơi khác người}
  \begin{block}{Ba bất thường ở baseline}
    ...
    \end{itemize}
  \end{block}
  \begin{exampleblock}{Ba câu hỏi $\Rightarrow$ ba mở rộng Turn này}
    \begin{itemize}\setlength\itemsep{4pt}
      ...
      \item Chỉ ở model \emph{mở / tự host}? \quad$\to$\quad \alert{thử model frontier đóng}
    \end{itemize}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Hai thí nghiệm --- tổng quan}
  ...
  \begin{tabular}{lll}
    ...
    Model     & \makecell[l]{Qwen 7B$\to$32B$\to$72B\\Gemma 9B$\to$27B, Llama 8B$\to$70B} & \makecell[l]{3 nhỏ + 4 lớn\\(32/72B, 27B, 70B)} \\
...
% ======================================================================
\section{Mở rộng 3 --- Model frontier (đóng)}

\begin{frame}{Bất thường có phải chỉ của model MỞ?}
  \begin{block}{Turn 1--2 toàn model mở, tự host}
    Qwen / Gemma / Llama đều là model \textbf{mở}, chạy offline. Biết đâu phớt lờ rủi ro + đóng dư chỉ là nét riêng của \emph{lớp model mở}?
  \end{block}
  \begin{exampleblock}{Thử một model ĐÓNG, frontier, khác nhà cung cấp}
    \begin{itemize}\setlength\itemsep{4pt}
...
\begin{frame}{Frontier đóng: vẫn đúng một bức tranh}
