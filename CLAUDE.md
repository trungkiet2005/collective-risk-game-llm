...
Read [paper/AAMAS/REPRODUCE.md](paper/AAMAS/REPRODUCE.md) for the current figure and validation commands. Figure 1 (since 19 September) is `figures/fig_game_designs.pdf`, the slate-mono colouring of the `tikz_figure_complete/figure_biolinum.tex` layout, which `tikz_figure_complete/palettes/build_palettes.sh` rebuilds and copies there (every figure `main.tex` includes lives in `paper/AAMAS/figures/`): the game and the four designs only, no findings panel; `TikZ_Figure_Deliverables/figure_rebuilt.pdf` is retired. Figure 3 (arrow plot since 19 September) joins the risk-0.1 and risk-0.9 answer and contribution levels by arrows; `fig_knowdo.csv` holds those levels, with the same intervals as the supplement's know-do table, whose Shift rows keep the contrasts. Figure 5 shows both comparison intervals (panel a arrows) and all five Grok compositions (bands). Selection is a rare-mutation Fermi process with population size varied, not an isolated scoring-rule intervention. The Qwen repair changes all of its last-round zeros, not one seat. Do not restore the older interpretations from planning notes. Missing group-goal, printed-pool and zero-risk-probe controls remain untested.
