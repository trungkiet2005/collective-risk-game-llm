...
def test_registered_estimates_unchanged(name, digest):
    assert hashlib.sha256((pub.FIG / f'{name}.csv').read_bytes()).hexdigest() == digest
...
def test_sources_are_deterministic_and_order_invariant(name, render, tmp_path, monkeypatch):
    with (pub.FIG / f'{name}.csv').open() as stream:
        rows = list(csv.DictReader(stream))
    monkeypatch.setattr(pub, 'FIG', tmp_path)
    first = render(rows).read_bytes()
    assert render(list(reversed(rows))).read_bytes() == first
