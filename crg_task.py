...
def _call_llm(llm, prompt, seed, max_attempts=6):
    """One free-text generation. Returns (text, usage); retries transient proxy errors."""
    for attempt in range(1, max_attempts + 1):
        try:
            with kbench.chats.new("turn", orphan=True) as chat:
                text = llm.prompt(prompt, temperature=TEMPERATURE, seed=seed)
...
def play_game(llm, risk, language, rep, model_tag, turns_sink):
    ...
    game_row = {
        ...
        "group_total": pot, "target": TARGET, "target_reached": int(target_met),
        "catastrophe": int(disaster), "mean_payoff": round(sum(payoffs) / N_PLAYERS, 4),
...
def collective_risk_baseline(llm) -> dict:
    ...
    out_dir = Path(os.environ.get(
        "CRG_OUT", f"frontier_results/{re.sub(r'[^A-Za-z0-9._-]', '_', model_tag)}/exp_baseline"))
