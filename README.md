...
├── kaggle/                   # mọi thứ chạy trên Kaggle
│   ├── experiments/          # notebook GPU offline (baseline, comprehension, framing, ...)
│   ├── setup/                # notebook chuẩn bị: build_quant_wheels, crsd_notebook
│   └── benchmarks/           # Kaggle Benchmarks (proxy frontier) + artifacts/ (.task/.run.json)
├── results/                  # phân tích & output
│   ├── scripts/              # analyze_*.py, make_*_figs.py (đọc data/ + frontier/)
│   └── frontier/             # kết quả model frontier (proxy): games.csv, turns.jsonl
├── docs/
...
1. **Wheels** (notebook Internet ON): chạy [kaggle/setup/build_quant_wheels.py](kaggle/setup/build_quant_wheels.py)
...
- Paper (AI-readable): [Milinski 2008](docs/papers/papers_markdown/milinski-2008-collective-risk.md) ·
  [FairGame](docs/papers/papers_markdown/fairgame-framework.md) ·
  [Related work](docs/papers/papers_markdown/related-work-llm-social-dilemma.md) ·
  [Language effects](docs/papers/papers_markdown/language-effects-llm.md)
